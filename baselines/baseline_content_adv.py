...
tf.flags.DEFINE_integer("display_freq", 200,
...
tf.flags.DEFINE_float("gan_weight", 0.0001, "weight on GAN term.")
...
Model = collections.namedtuple("Model",
                               "image_outputs,"
                               "gen_loss_GAN,"
                               "gen_loss_content_L1, gen_loss, discrim_loss,"
                               "train, global_step")
...
def create_discriminator(discrim_inputs, discrim_targets):
  ...
  # layer_6: [batch, 8, 6, ndf * 8] => [batch, 4, 3, ndf * 8]
  stride = 2
  for i in range(n_layers):
    with tf.variable_scope("layer_%d" % (len(layers) + 1)):
      out_channels = FLAGS.ndf * min(2**(i+1), 8)
  ...
  # layer_7: [batch, 4, 3, ndf * 8] => [batch, 1, 1, 1]
  with tf.variable_scope("layer_%d" % (len(layers) + 1)):
    convolved = final_conv(rectified, out_channels=1, stride=1)
...
def create_model(product_image, body_seg, skin_seg, pose_map, prod_seg, image):
  ...
  with tf.name_scope("real_discriminator"):
    with tf.variable_scope("discriminator"):
      ...
      predict_real = create_discriminator(product_image, image)

  with tf.name_scope("fake_discriminator"):
    with tf.variable_scope("discriminator", reuse=True):
      # 2x [batch, height, width, channels] => [batch, 30, 22, 1]
      predict_fake = create_discriminator(product_image, image_outputs)
  ...
  with tf.name_scope("generator_loss"):
    ...
    gen_loss = (gen_loss_content_L1 * FLAGS.content_l1_weight + 
               gen_loss_GAN * FLAGS.gan_weight)
      ...
      for step in range(max_steps):
        ...
        if should(FLAGS.display_freq):
          ...
          filesets = save_images(results["display"],
                                 image_dict=["image", "product_image",
                                             "body_segment", "prod_segment",
                                             "skin_segment", "image_outputs", ],
