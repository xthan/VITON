...
import collections
from baseline_content_adv import create_model
import os
import time
...
def _process_image(image_name, product_image_name, sess,
  ...
  [image, prod_image, body_segment, prod_segment, skin_segment] = sess.run([image, prod_image, body_segment, prod_segment, skin_segment])
...
def main(unused_argv):
  ...
  model = create_model(prod_image_holder, body_segment_holder, skin_segment_holder,
  ...
  with tf.Session() as sess:
    ...
    for i in range(FLAGS.begin,FLAGS.end,batch_size): # range(0, len(test_info) - 16, 16):
      ...
      for j in range(i, i + batch_size):
        ...
        ##########################################################
        # product_image_name = info[0][:-5] + "1.jpg" # info[1]
        # product_image_name = info[1]
      ...
      [image_output, loss, step] = sess.run(
                                [model.image_outputs,
      ...
      for j in range(batch_size):
        scipy.misc.imsave(FLAGS.result_dir + ("images/%08d_" % step) + image_names[j] + "_" + product_image_names[j] + '.png', (image_output[j] / 2.0 + 0.5))
        scipy.misc.imsave(FLAGS.result_dir + "images/" + image_names[j], (images[j] / 2.0 + 0.5))
        scipy.misc.imsave(FLAGS.result_dir + "images/"+ product_image_names[j], (prod_images[j] / 2.0 + 0.5))
